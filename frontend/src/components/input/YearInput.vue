...
const shouldShowValidationState = computed(() =>
  Boolean(localModelValue.value && (inputCompleted.value || userHasFinished.value)),
)
...
const inputCompleted = computed(
  () => localModelValue.value && new Mask({ mask }).completed(localModelValue.value),
)
...
function backspaceDelete() {
  emit('update:validationError', undefined)
  if (localModelValue.value === '') emit('update:modelValue', localModelValue.value)
}
...
<template>
  <TextInput
    :id="id"
    v-maska="mask"
    :ariaLabel="($attrs.ariaLabel as string) ?? ''"
