...
function maybeFocusPrevious() {
  if (chipsInput.value?.selectionStart === 0) {
    focusedChip.value = props.modelValue !== undefined ? props.modelValue.length - 1 : undefined
  }
}
...
async function determineInputWidth() {
  ...
  const { borderLeftWidth, borderRightWidth } = getComputedStyle(chipsInput.value as Element)
...
<template>
  <div class="w-full">
    ...
    <div
      ref="wrapperEl"
      class="shadow-blue flex min-h-48 w-full cursor-text flex-wrap bg-white p-8 shadow-button shadow-blue-800"
    ...
    >
      ...
      <span v-if="!readOnly" class="ml-8 flex max-w-full flex-auto items-center justify-start">
