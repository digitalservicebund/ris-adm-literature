...
export function useCollapsingMenuBar(buttons: Ref<EditorButton[]>, maxBarEntries: Ref<number>) {
  const collapsedButtons = computed(() => {
    ...
    while (buttonList.length > maxBarEntries.value) {
      ...
      const lastGroupName =
        collapsableButtonsWithGroup[collapsableButtonsWithGroup.length - 1].group;

      const buttonsOfGroup = buttonList.filter((button) => button.group == lastGroupName);

      const menuButtonIndex = buttonList.indexOf(buttonsOfGroup[0]);

      buttonList = buttonList.filter((button) => !buttonsOfGroup.includes(button));
