...
class FieldOfLawSpecificationIntegrationTest {
  ...
  void toPredicate_withTextTerms() {
    // given
    FieldOfLawSpecification specification = new FieldOfLawSpecification("AR", List.of("term1", "term2"), List.of());
    CriteriaBuilder criteriaBuilder = entityManager.getEntityManager().getCriteriaBuilder();
    CriteriaQuery<FieldOfLawEntity> query = criteriaBuilder.createQuery(FieldOfLawEntity.class);
    Root<FieldOfLawEntity> root = query.from(FieldOfLawEntity.class);
    ...
    // then
    assertThat(sql).contains("where fole1_0.notation='NEW' and lower(fole1_0.text) like ? escape '' and lower(fole1_0.text) like ? escape '' and fole1_0.identifier like ? escape ''");
  }

  @Test
  void toPredicate_withNormTerms() {
    // given
    FieldOfLawSpecification specification = new FieldOfLawSpecification("AR", List.of(),
      List.of("norm1", "norm2"));
    CriteriaBuilder criteriaBuilder = entityManager.getEntityManager().getCriteriaBuilder();
    CriteriaQuery<FieldOfLawEntity> query = criteriaBuilder.createQuery(FieldOfLawEntity.class);
    Root<FieldOfLawEntity> root = query.from(FieldOfLawEntity.class);
  ...
  void toPredicate_withTextAndNormTerms() {
    // given
    FieldOfLawSpecification specification = new FieldOfLawSpecification("AR", List.of("term1", "term2"),
      List.of("norm1", "norm2"));
    CriteriaBuilder criteriaBuilder = entityManager.getEntityManager().getCriteriaBuilder();
    CriteriaQuery<FieldOfLawEntity> query = criteriaBuilder.createQuery(FieldOfLawEntity.class);
    Root<FieldOfLawEntity> root = query.from(FieldOfLawEntity.class);
