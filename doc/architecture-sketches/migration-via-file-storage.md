...
```mermaid
C4Container
    title RIS-VwV, Container Diagram with Migration via File Storage 🚧 Work in Progress! 🚧
    ...
    System_Boundary("ris-vwv", "RIS-VwV"){
        ...
        Container("fileStorage", "File Storage", "??? but already exists<br> in the NeuRIS space" "Stores published VwV and BSG dump")
        Rel("backend", "fileStorage", "Stores published VwV", "HTTPS")
        Rel("neurisPerson", "fileStorage", "Uploads (partial) BSG dump")
    }
